...
%\subsection*{Candidate genes ananlysis}
%We looked for variants with high and moderate effect in the set of genes identified in the only study similar to ours 
%3 variants high impact and  174 with moderate effect 
%Two embryos (AS064 and AS087) share a missense mutation in the \textit{HSPD1} gene. %It is located on chromosome 2:197,486,584-197,516,737 reverse strand. 
%\textit{HSPD1} codes for heat shock protein 60 (HSP60) that forms together with heat shock protein 10 (HSP10)double-barrel chaperonin complexes that are essential for folding to the native state of proteins in the mitochondrial matrix space. \cite{bross2016disease}. 

%variants in 254 of 275  genes (21 genes no annotated by vep) 
%3 variant high impact 
%- HSPD1  frameshift  deletion 1 nucleotide in AS093, this variant was not selected by our pipeline as the transcript in which it is found non ha pLI, variant misses CADD and is only in one of the lists (DDDD) rare  

%also from pipeline in samples AS087, AS064 prioritization  
